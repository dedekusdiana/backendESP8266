/*
  ...
  3. AUTO (jadwal lampu) -- jadwal nyala/mati (field auto1 & auto2 dari database) diterima
     ...
     ESP melapor ke backend (POST /api/data) supaya status di app ikut berubah.
...
const int led = 0;  // LED bawaan ESP-01 (GPIO0, aktif LOW) = Relay 1
...
// Lapor status relay ke backend (POST /api/data). Return true kalau sukses.
